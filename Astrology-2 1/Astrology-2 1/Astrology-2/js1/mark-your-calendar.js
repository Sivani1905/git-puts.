...
(function($) {
    ...
    $.fn.markyourcalendar = function(opts) {
        ...
        this.getDatesHeader = function() {
            var tmp = ``;
            for (i = 0; i < 7; i++) {
                var d = settings.startDate.addDays(i+1);
                //console.log('Mark Calender - '+d.getMonth());
                const month = ["Jan","Feb","Mar","Apr","May","June","July","Aug","Sep","Oct","Nov","Dec"];
                tmp += `
                    <div class="myc-date-header" id="myc-date-header-` + i + `" data-date="` + formatDate(settings.startDate.addDays(i+1)) + `">
                        <div class="myc-date-month">` + month[d.getMonth()] + `</div>
